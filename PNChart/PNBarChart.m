    ...
    if (_showLabel)
    {
        ...
        for (int index = 0; index < _yLabelSum; index++)
        {
            ...
            PNChartLabel* label = [[PNChartLabel alloc]
                initWithFrame:CGRectMake(0, yLabelSectionHeight * index + _chartMargin - yLabelHeight / 2.0,
                                         _yChartLabelWidth, yLabelHeight)];
    ...
    if (_showLabel)
    {
        ...
        for (int index = 0; index < _xLabels.count; index++)
        {
            ...
            if (labelAddCount == _xLabelSkip)
            {
                ...
                label.center = CGPointMake(labelXPosition, self.frame.size.height - xLabelHeight - _chartMargin
                                                           + label.frame.size.height / 2.0 + _labelMarginTop);
                labelAddCount = 0;
    ...
    for (NSNumber* valueString in _yValues)
    {
        ...
        else
        {
            ...
            bar = [[PNBar alloc] initWithFrame:CGRectMake(barXPosition, // Bar X position
                                                          self.frame.size.height - chartCavanHeight - xLabelHeight
                                                          - _chartMargin, // Bar Y position
                                                          barWidth, // Bar witdh
...
- (void)strokeChart
{
    ...
    if (_showChartBorder)
    {
        ...
        [progressline addLineToPoint:CGPointMake(self.frame.size.width - _chartMargin,
                                                 self.frame.size.height - xLabelHeight - _chartMargin)];
