       ...
       overrideLineWidth:(NSNumber*)overrideLineWidth
{
    ...
    if (self)
    {
        ...
        UIBezierPath* circlePath = [UIBezierPath
            bezierPathWithArcCenter:CGPointMake(self.bounds.size.width / 2.0f, self.bounds.size.height / 2.0f)
        ...
        _circleBackground.strokeColor
            = (hasBackgroundShadow ? backgroundShadowColor.CGColor : [UIColor clearColor].CGColor);
...
- (void)updateChartByCurrent:(NSNumber*)current
{
    ...
    if (_displayCountingLabel)
    {
        [self.countingLabel countFrom:fmin([_current floatValue], [_total floatValue])
                                   to:fmin([current floatValue], [_total floatValue])
