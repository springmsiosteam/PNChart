...
- (void)setupDefaultValues
{
    ...
    //    self.frame = CGRectMake((SCREEN_WIDTH - self.frame.size.width) / 2, 200, self.frame.size.width,
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            for (PNScatterChartData* chartData in data)
            {
                for (NSUInteger i = 0; i < chartData.itemCount; i++)
                {
                    ...
                    if (!(xValue >= _AxisX_minValue && xValue <= _AxisX_maxValue)
                        || !(yValue >= _AxisY_minValue && yValue <= _AxisY_maxValue))
                    {
                    ...
                    CAShapeLayer* shape =
                        [self drawingPointsForChartData:chartData AndWithX:xFinilizeValue AndWithY:yFinilizeValue];
...
- (CAShapeLayer*)drawingPointsForChartData:(PNScatterChartData*)chartData AndWithX:(CGFloat)X AndWithY:(CGFloat)Y
{
    if (chartData.inflexionPointStyle == PNScatterChartPointStyleCircle)
    {
        ...
        circle.path =
            [UIBezierPath bezierPathWithRoundedRect:CGRectMake(X - radius, Y - radius, 2.0 * radius, 2.0 * radius)
...
- (void)drawLineFromPoint:(CGPoint)startPoint
             ...
             WithLineWith:(CGFloat)lineWidth
